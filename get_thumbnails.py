                    ...
                    if len(data) > 8 and data[0] == 0xF1:
                        if data[1] == 0xD1: # Protocol ACK
                            if (len(data) >= 10 and data[9] == seq) or data[7] == seq: return True
                             ...
                             # FIX: Egal was drin steht, wir haben eine Antwort.
                             # Bei Text ACK ist das Paket erledigt.
                             if len(data) >= 11 and data[8:11] == b'ACK': return True
                             pass # Bei echten Daten warten wir im wait_for_data loop
        ...
        while time.time() - start < timeout:
            try:
                ...
                if len(data) > 8 and data[0] == 0xF1 and data[1] == 0xD0:
                    # FIX: ZUERST BESTÄTIGEN!
                    # Jedes D0 Paket braucht ein D1 ACK, sonst flippt die Kamera aus.
                    rx_seq = data[7]
                    ...
                    # Jetzt Inhalt prüfen
                    payload = data[8:]
                    if payload.startswith(b'ACK'): continue # Nur Text-ACK, weiter warten
